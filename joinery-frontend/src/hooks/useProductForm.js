...
export const getInitialProductFormValues = (product) => ({
  ...
  price_per_unit_in_cents: product?.price_per_unit_in_cents ? product.price_per_unit_in_cents / 100 : 0,
  available_volume: product?.available_volume || 1,
  pricing_unit: product?.pricing_unit || 'each',
  min_order_unit: product?.min_order_unit || 1,
...
export const useProductForm = (product = null) => {
  return useForm({
    ...
    validate: {
      ...
      price_per_unit_in_cents: (value) => (value >= 0 ? null : 'Price must be non-negative'),
      available_volume: (value) => (value >= 0 ? null : 'Stock must be a non-negative integer'),
      flat_rate: {
